 ...
 *      -PROBLEMA: devo usare heapsize-1 per c+accedere a A[heapszie]!!!
 *
...
void min_heapify(struct nodo *A, int posiz){
    ...
    if(r<*(A->heapsizep) && (A+r)->dist<(A+min)->dist) {
        min = r;
    }
    if(min!=posiz){
        struct nodo swap;
...
struct nodo heap_extract_min(struct nodo *A){
    ...
    minimo = *A; //min=A[0];
    *A = *(A + *(A->heapsizep)-1);     //A[0]=[A.heapsize]
    *(A->heapsizep) = (*(A->heapsizep))-1;
    min_heapify(A,0);
    return minimo;
}
...
void dijkstra(int dim, int mat[dim][dim], struct nodo *A, struct nodo *G){
    ...
    *(A->heapsizep)=0;   //inizializzo Q=0
    ...
    //test ricerca errore
    while(*(A->heapsizep)>0){   //DUBBIO SU QUESTA CONDIZOONE
